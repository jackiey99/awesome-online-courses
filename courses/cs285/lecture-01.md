...
*Slide 2 · 机器人抓取的两种思路。左边：7-DoF 机械臂 + 单目相机的硬件设置。右上 Option 1：手工设计 — 理解问题、为每种物体形态写规则。右下 Option 2：作为监督学习 — 从 (image, grasp) 数据集学映射。两种都各有坑。*
...
*Slide 4 · RL 解决机器人抓取的范式。机器人采集 `(image, action, {success, failure})` 数据 → RL 算法学习 → 改进策略 → 继续采集。这是一个 **闭环** 而非监督学习的单向 pipeline。*
...
*Slide 6 · 主流生成式 AI 的统一视角：都在估计 $p_\theta(\mathbf{x})$（左下，无条件分布）或 $p_\theta(\mathbf{y} \mid \mathbf{x})$（右下，条件分布）。无论 LLM 还是文生图，骨子里都是密度估计的极致放大版。*
...
*Slide 7 · RL 的两条历史血脉。左上是 B. F. Skinner（心理学/行为主义，给我们 agent-environment-reward 框架）。下方是 Karl Sims 1994 的进化虚拟生物 + Yuval Tassa 的最优控制（控制论/优化）。右侧是它们融合后的 deep RL：神经网络 + Agent-Environment 循环，催生出 AlphaGo 这类系统。*
...
*Slide 8 · 两种 "impressive" 的本质区别。左 — AlphaGo Move 37 的 impressive 来自 **"没有人会下那一步"**（emergence）；右 — 生成式 AI 的 impressive 来自 **"它产出的看上去像人画的"**（imitation）。这两种 impressive 指向完全不同的研究方向。*
...
*Slide 15 · 监督学习 vs RL 形式化对照。左 — 监督学习：输入 $\mathbf{x}$，输出 $\mathbf{y}$，目标 $f_\theta(\mathbf{x}_i) \approx \mathbf{y}_i$。右 — RL：每个时间步看到 $\mathbf{s}_t$，输出 $\mathbf{a}_t$，目标学一个 policy $\pi_\theta : \mathbf{s}_t \to \mathbf{a}_t$ 来最大化 $\sum_t r_t$。"pick your own actions" 这一点尤其关键。*
...
*Slide 21 · Cathy Wu 的交通 RL 实验。左 — 环形跑道：人类驾驶模型自发产生 traffic jam（速度波动会被放大成停-走波）。右 — 8 字形交叉口同理。红色车是 RL agent，目标是最大化整体平均速度。*
...
*Slide 24 · RLHF 流程。从 prompt 池采样 → 初始 LM 生成多个回复 → 人类对回复打分（ranking）→ 训练一个 reward model → 用 RL（PPO）让 LM 最大化 reward。这是把"人类偏好"注入 LLM 的关键手段。*
...
*Slide 30 · Sutton 的核心论断：能 arbitrary scale 的两件事是 **learning** 和 **search**。Learning（从数据中提取 pattern → 理解世界）和 Search（用计算从理解中提取推断 → 涌现行为）**缺一不可**。Deep learning 单独不够，optimization 离开数据也走不通真实世界。*
...
*Slide 38 · 这门课的精神 quote — 来自 Turing 1950 年的 *Computing Machinery and Intelligence*。意思是：与其编程模拟成人的心智（写出所有规则），不如造一个能"学"的婴儿大脑，让它经历教育自然变成成人大脑。这恰恰是 deep RL 的研究范式。*
